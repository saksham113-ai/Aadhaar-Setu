...
export const generateInitialData = (): AadhaarRecord[] => {
  const allData: AadhaarRecord[] = [];
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
  const ageGroups: ('0-18' | '19-45' | '46-60' | '60+')[] = ['0-18', '19-45', '46-60', '60+'];

  Object.entries(STATES_DISTRICTS).forEach(([state, districts]) => {
    districts.forEach(district => {
      const enrolled = Math.floor(Math.random() * 5000) + 500;
      const updates = Math.floor(Math.random() * enrolled * 0.4) + 100;
      const rejected = Math.floor(Math.random() * (enrolled + updates) * 0.05);
      ...
      allData.push({
        ...
        updates,
        rejected,
        gender_m: Math.floor(enrolled * (0.48 + Math.random() * 0.04)),
        gender_f: Math.floor(enrolled * (0.47 + Math.random() * 0.04)),
...
export const MOCK_DATA = generateInitialData();

export const UI_COLORS = {
  primary: '#003366', // Deep Navy (UIDAI Theme)
